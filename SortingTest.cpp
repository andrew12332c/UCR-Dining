...
void sortAToZ(vector<RestaurantDish*>& restaurantDishes);
void sortZToA(vector<RestaurantDish*>& restaurantDishes);
...
void chooseSort(vector<RestaurantDish*>& restaurantDishes) {
    ...
    cout << "7. A-Z" << endl << endl;
    cout << "8. Z-A" << endl << endl;
...
void sortAToZ(vector<RestaurantDish*>& restaurantDishes) {
    for (int i = 0; i < restaurantDishes.size(); ++i) {
        for (int j = i + 1; j < restaurantDishes.size(); ++j) {
            if (restaurantDishes.at(i)->getName() > restaurantDishes.at(j)->getName()) {
                swap(restaurantDishes.at(i), restaurantDishes.at(j)); // can use temp alternatively to swap
            }
        }
    }
...
void createRestaurantDishes(vector<RestaurantDish*>& restaurantDishes) {
    RestaurantDish *burgerDish = new RestaurantDish("Burger", 500, 20, 8);
    restaurantDishes.push_back(burgerDish);

    RestaurantDish *steakDish = new RestaurantDish("Steak", 410, 50, 10);
    ...
    RestaurantDish *curryDish = new RestaurantDish("Chicken Curry", 360, 25, 9);
    restaurantDishes.push_back(curryDish);

    RestaurantDish *spaghettiDish = new RestaurantDish("Spaghetti and Meatballs", 380, 35, 6);
